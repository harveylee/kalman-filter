...
test('Init with zero mean', () => {
	...
	expect(kf1.predict() && Array.isArray(kf1.predict().mean) && Array.isArray(kf1.predict().covariance)).toBeTruthy();
});

// Test 2: Verify that smalls previousCorrected.covariance and dynamic.covariance
// return a small predicted.covariance

test('Impact previousCorrected and dynamic covariance', () => {
...
test('Huge predicted covariance', () => {
...
test('Observation covariance test', () => {
...
test('Predicted covariance equals to zero', () => {
	const kf = new CoreKalmanFilter(defaultOptions);
	const predicted = new State({
		mean: [[0]],
		covariance: [
