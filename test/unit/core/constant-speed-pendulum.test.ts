...
test('Fitted observation', () => {
	...
	expect(corrected1 && Array.isArray(corrected1.mean) && Array.isArray(corrected1.covariance)).toBeTruthy();
	expect(corrected2 && Array.isArray(corrected2.mean) && Array.isArray(corrected2.covariance)).toBeTruthy();
...
test('Covariance between position and speed', () => {
...
test('Bad fit observation and correlation', () => {
	const predicted1 = new State({
		mean: [[0.1], [0.5]],
		covariance: [
			[0.1, 0.0001],
